...
foreach ($xml->user as $user) {
    if ((string)$user->username === $username) {
        // Loop through the rentals to find movies with returnStatus 'no'
        foreach ($user->rentals->rental as $rental) {
            if ((string)$rental->returnStatus === 'no') {
                // Add relevant rental data to the array
                $rentalHistory[] = [
...
echo '<h3>Currently Rented Movies</h3>'; // Changed the heading to reflect the current rentals
echo '<table>';
